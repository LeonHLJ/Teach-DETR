...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        out_pred_scores = F.sigmoid(class_logits / temperature)        

        # split boxes and scores per image
        # 根据每张图像的预测bbox数量分割结果
        pred_boxes_list = pred_boxes.split(boxes_per_image, 0)
        pred_scores_list = pred_scores.split(boxes_per_image, 0)
        out_pred_scores_list = out_pred_scores.split(boxes_per_image, 0)
        ...
        all_labels = []
        # 遍历每张图像预测信息
        for boxes, scores, out_pred_scores, image_shape in zip(pred_boxes_list, pred_scores_list, out_pred_scores_list, image_shapes):
            ...
            boxes = boxes[:, 1:]
            scores = scores[:, 1:]
            out_pred_scores = out_pred_scores[:, 1:]
            labels = labels[:, 1:]
            ...
            indexs = torch.arange(scores.shape[0], device=device)
            ...
            boxes, scores, labels, indexs = boxes[inds], scores[inds], labels[inds], indexs[inds]

            # remove empty boxes
            # 移除小目标
            keep = box_ops.remove_small_boxes(boxes, min_size=1.)
            boxes, scores, labels, indexs = boxes[keep], scores[keep], labels[keep], indexs[keep]
            ...
            boxes = box_ops.box_xyxy_to_cxcywh(boxes / scale_fct[None, :])

            boxes_indexs = indexs // (num_classes - 1)
            boxes_class_scores = out_pred_scores[boxes_indexs]

            all_boxes.append(boxes)
            all_scores.append(scores)
            all_class_scores.append(boxes_class_scores)
            all_labels.append(labels)

        return all_boxes, all_scores, all_class_scores, all_labels
