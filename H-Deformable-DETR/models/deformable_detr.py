...
from mask_rcnn.model import build_mask_rcnn
...
def ce_loss(pred, target, temp_p, temp_t):
    soft_target = torch.softmax(target / temp_t, dim=1)
    log_soft_pred = torch.log_softmax(pred / temp_p, dim=1)
    loss = -1.0 * (soft_target * log_soft_pred).sum(1).mean()
    return loss
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25):
        ...
        self.aux_losses = copy.deepcopy(self.losses)
        if 'logit_distill' in self.aux_losses:
            self.aux_losses.remove('logit_distill')

        if 'feat_distill' in self.aux_losses:    
            self.aux_losses.remove('feat_distill')
    ...
    def loss_weighted_labels(self, outputs, targets, indices, num_boxes, log=True):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert "pred_logits" in outputs
        ...
        target_scores_o = torch.cat(
            [t["scores"][J] for t, (_, J) in zip(targets, indices)]
        )
        ...
        target_scores[idx] = target_scores_o
        ...
        loss_ce = ((
            ...
            * target_scores[..., None]).mean(1).sum() * src_logits.shape[1]
        )
    ...
    def loss_weighted_boxes(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, h, w), normalized by the image size.
        """
        ...
        target_scores_o = torch.cat(
            [t["scores"][J] for t, (_, J) in zip(targets, indices)], dim=0
        )

        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction="none") * target_scores_o[:, None]
    ...
    def get_loss(self, loss, inputs, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
        ...
        if 'distill' in loss:
            return loss_map[loss](inputs, outputs, targets, indices, num_boxes, **kwargs)
        else:
            return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def get_aux_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            "labels": self.loss_labels,
            "cardinality": self.loss_cardinality,
            "boxes": self.loss_boxes,
    ...
    def get_ensemble_loss(self, loss, inputs, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            "labels": self.loss_weighted_labels,
            "cardinality": self.loss_cardinality,
            "boxes": self.loss_weighted_boxes,
            "masks": self.loss_masks,
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
    ...
    def get_ensemble_aux_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            "labels": self.loss_weighted_labels,
            "cardinality": self.loss_cardinality,
            "boxes": self.loss_weighted_boxes,
    ...
    def forward(self, inputs, outputs, targets, is_one2many=False, is_ensemble=False):
        ...
        for loss in self.losses:
            if is_one2many and loss == 'feat_distill':
                continue
            kwargs = {}
            if is_ensemble:
                losses.update(self.get_ensemble_loss(loss, inputs, outputs, targets, indices, num_boxes, **kwargs))
            else:
                losses.update(self.get_loss(loss, inputs, outputs, targets, indices, num_boxes, **kwargs))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.aux_losses:
                    ...
                    if is_ensemble:
                        l_dict = self.get_ensemble_aux_loss(loss, aux_outputs, targets, indices, num_boxes, **kwargs)
                    else:
                        l_dict = self.get_aux_loss(loss, aux_outputs, targets, indices, num_boxes, **kwargs)
...
def build(args):
    ...
    for key, value in weight_dict.items():
        ...
        new_dict[key + "_ensemble"] = value
        new_dict[key + "_one2many_ensemble"] = value
    ...
    criterion = SetCriterion(
        num_classes, matcher=matcher, weight_dict=weight_dict, losses=losses, focal_alpha=args.focal_alpha
    )
    criterion.to(device)
